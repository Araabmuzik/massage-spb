    ...
    btn.addEventListener("click", () => {
      ...
      if (keyForResultsData) {
        ...
        <div class="quiz-result">
          <h2>${resultsData[keyForResultsData].title}</h2>
          <p>${resultsData[keyForResultsData].description}</p>
          <a href="tel:+799999" class="button" >📞 ${resultsData[keyForResultsData].button}</a>
        </div>
      `;
      } else {
        // Fallback на случай, если результат не найден
        quizFinalScreen.innerHTML = `
        <div class="quiz-result">
